...
def process_huge_dataset
  ...
  writer = MemoryEfficientXMLWriter.new('output/huge_dataset.xml', 'users')
  ...
  puts "✓ Successfully created output/huge_dataset.xml"

  # Show file size
  file_size = File.size('output/huge_dataset.xml')
...
def process_custom_data
  ...
  MemoryEfficientXMLWriter.new('output/business_data.xml', 'companies').write_xml do |writer|

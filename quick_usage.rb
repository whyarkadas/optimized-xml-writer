...
writer = MemoryEfficientXMLWriter.new('users.xml', 'users')
writer.write_complete_xml(user_data, 'user')

puts "✓ Created users.xml from array of #{user_data.length} hashes"
...
writer = MemoryEfficientXMLWriter.new('large_users.xml', 'users')
...
puts "✓ Created large_users.xml with #{total_processed} records"
...
writer = MemoryEfficientXMLWriter.new('complex_data.xml', 'customers')
writer.write_complete_xml(complex_data, 'customer')

puts "✓ Created complex_data.xml with nested structures"
...
batch_writer = BatchXMLWriter.new('huge_dataset.xml', 'records', batch_size: 1000)
...
puts "✓ Created huge_dataset.xml with 50,000 records using batch processing"
...
writer = MemoryEfficientXMLWriter.new('catalog.xml', 'product_catalog')
writer.start_document
...
writer.finish_document
puts "✓ Created catalog.xml with custom element names"
...
files = Dir.glob('*.xml').sort
files.each do |file|
  size = File.size(file)
  size_mb = (size / 1024.0 / 1024.0).round(3)
  puts "  #{file.ljust(20)} - #{size_mb.to_s.rjust(8)} MB"
end
